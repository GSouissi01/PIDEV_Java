...
nameLabelHeader.setStyle("-fx-font-size: 20px; -fx-font-weight: bold");
libelle.setStyle("-fx-font-size: 20px; -fx-font-weight: bold");
priceLabelHeader.setStyle("-fx-font-size: 20px; -fx-font-weight: bold");
quantityLabelHeader.setStyle("-fx-font-size: 20px; -fx-font-weight: bold");
actionsLabelHeader.setStyle("-fx-font-size: 20px; -fx-font-weight: bold");
...
nameLabel.setStyle("-fx-font-weight: bold"); // set bold font weight for nameLabel

priceLabel.setFont(new Font(18)); // set font size of priceLabel to 18
priceLabel.setStyle("-fx-font-weight: bold"); // set bold font weight for priceLabel

quantityLabel.setFont(new Font(18)); // set font size of quantityLabel to 18
quantityLabel.setStyle("-fx-font-weight: bold"); // set bold font weight for quantityLabel
...
validateButton.setOnAction(e -> {
    ...
    // Create a string to store information about the products in the cart
    String productList = "Liste des produits dans le panier : \n\n";
    
    // Iterate through the cart items to get the product information
    for (CartItem cartItem : cartItems) {
        ...
        // Add the product information to the string
        productList += "Nom du produit : " + product.getLibelle() + "\n";
        productList += "Prix unitaire : " + product.getPrix() + "\n";
        productList += "Quantité commandée : " + quantity + "\n\n";
    }
...
Security.setProperty("jdk.tls.client.cipherSuites", "TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256");


        Session session = Session.getInstance(properties, new Authenticator() {
            @Override
            protected PasswordAuthentication getPasswordAuthentication() {
                return new PasswordAuthentication("pidev2223@gmail.com", "ykczxfhxnhnufzhk");
            }
        });
        try {
            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress("pidev2223@gmail.com"));
            message.setRecipients(Message.RecipientType.TO, InternetAddress.parse("azizbramlihlel@gmail.com"));
            message.setSubject("Commande validée");
            
            // Add the product list to the message body
            message.setText("Votre commande a été validée avec succès.\n\n" + productList);
           
            Transport.send(message);
            
            Alert alert1 = new Alert(AlertType.INFORMATION);
            alert1.setTitle("Envoi de mail");
            alert1.setHeaderText(null);
            alert1.setContentText("Mail envoyé avec succès !");
            alert1.showAndWait();
